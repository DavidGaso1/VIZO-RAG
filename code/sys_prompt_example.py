...
def print_instructions(config_name: str):
    print("\n" + "="*80)
    print(f"Interactive Q&A Assistant with System Prompt — VIZO Chat Assistant 📝")
...
def run_interactive_conversation_with_system_prompt(
    ...
    print(f"\nInteractive Q&A Assistant with System Prompt — VIZO Chat Assistant 📝")
    ...
    save_text_to_file(
        transcript,
        os.path.join(OUTPUTS_DIR, f"vizo_prompt_{system_prompt_config_name}.md"),
        header=f"VIZO System Prompt Example - {system_prompt_config_name}"
    )
...
def main() -> None:
    ...
    try:
        ...
        vae_publication_id = 'vizo_product_manual'
